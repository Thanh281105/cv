...
- Đánh giá thêm `grounding_dino` và `qwen2_5_vl_3b` baselines trên cùng 4,000 query.
...
### 6. Evaluate Grounding DINO và Qwen2.5-VL baselines

Ba model LocateAnything cũ đã được publish trong `thanhhoangnvbg/viground-contrast-results`, nên không cần chạy lại. Trước khi chạy baseline mới trên Vertex AI, sync artifact cũ về `artifacts/`:

```bash
python scripts/sync_hf_results.py
```

Trước khi chạy full benchmark, chạy smoke test để xác nhận output có cùng box format với LocateAnything:

```bash
python scripts/evaluate_baselines.py --model-key grounding_dino --limit-standard 5 --limit-hard-pairs 5 --box-threshold 0.2 --text-threshold 0.15 --smoke-check
python scripts/evaluate_baselines.py --model-key qwen2_5_vl_3b --limit-standard 5 --limit-hard-pairs 5 --smoke-check
```

Smoke test sẽ fail nếu prediction không parse được hoặc `raw` không có dạng:
...
```text
artifacts/evaluation/grounding_dino/smoke_box_format_check.json
artifacts/evaluation/qwen2_5_vl_3b/smoke_box_format_check.json
```

Sau khi smoke test pass, chạy full benchmark. Các baseline này dùng cùng dataset đã pin và mặc định chạy:

- 1,000 standard EN queries
- 1,000 standard VI queries
- 500 hard pairs EN, tương đương 1,000 hard-pair EN queries
- 500 hard pairs VI, tương đương 1,000 hard-pair VI queries

Tổng cộng mỗi baseline chạy 4,000 query. Không truyền `--limit-standard` hoặc `--limit-hard-pairs` cho final Vertex AI run.

Qwen2.5-VL baseline mặc định dùng visual token cap tương đương 2048:

```text
--qwen-max-pixels 1605632
```

Trong đó `1605632 = 2048 * 28 * 28`.

Grounding DINO:

```bash
python scripts/evaluate_baselines.py --model-key grounding_dino
```

Qwen2.5-VL-3B-Instruct:

```bash
python scripts/evaluate_baselines.py --model-key qwen2_5_vl_3b
```

Output:

```text
artifacts/evaluation/grounding_dino/
artifacts/evaluation/qwen2_5_vl_3b/
```

### 7. Tính thống kê và sinh bảng

Sau khi có đủ `predictions.jsonl` cho các model cần báo cáo:
...
- Notebook `Final_CV.ipynb` và `test_base_model.ipynb` dùng cho exploration/kiểm thử, còn pipeline chính nằm trong `scripts/`.
