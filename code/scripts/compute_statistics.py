...
CORE_MODELS = ("base", "random_ft", "hardpair_ft")
OPTIONAL_MODELS = ("grounding_dino", "qwen2_5_vl_3b")
...
def main() -> None:
    ...
    prediction_paths = {model: artifact_root / "evaluation" / model / "predictions.jsonl" for model in CORE_MODELS}
    ...
    for model in OPTIONAL_MODELS:
        path = artifact_root / "evaluation" / model / "predictions.jsonl"
        if path.exists():
            prediction_paths[model] = path
    ...
    standard = {model: standard_values(vi_standard, predictions[model]) for model in prediction_paths}
    hard = {model: hard_pair_values(hard_pairs, predictions[model]) for model in prediction_paths}
    ...
    if "grounding_dino" in prediction_paths:
        effects.extend(
            [
                compare("Grounding DINO versus Base", "acc05", standard["base"], standard["grounding_dino"], args.resamples, True),
                compare("Grounding DINO versus Base", "pair_accuracy", hard["base"], hard["grounding_dino"], args.resamples, True),
            ]
        )
    if "qwen2_5_vl_3b" in prediction_paths:
        effects.extend(
            [
                compare("Qwen2.5-VL-3B versus Base", "acc05", standard["base"], standard["qwen2_5_vl_3b"], args.resamples, True),
                compare("Qwen2.5-VL-3B versus Base", "pair_accuracy", hard["base"], hard["qwen2_5_vl_3b"], args.resamples, True),
            ]
        )
